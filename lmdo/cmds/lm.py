...
class Lm(Base):
    ...
    def get_pkg_name(self, file_type='.zip'):
        ...
        surfix = self.config_loader.get_value(
            'Stage') + '-' + self.config_loader.get_value('Service')
    ...
    def upload(self):
        ...
        if len(lambda_bucket) > 0:
            if not self.if_bucket_exist(lambda_bucket):
                Oprint.warn(
                    'bucket ' + lambda_bucket + " doesn't exist!", 's3')
                sys.exit(0)
        # Create a new bucket
        else:
        ...
        Oprint.info('Start uploading ' + self.get_pkg_name() +
                    ' to S3 bucket ' + self.config_loader.get_value('LambdaBucketName'), 's3')

        pkg_path = tmp_dir + self.get_pkg_name()
        progress = tqdm(
            total=float(os.stat(pkg_path).st_size), unit_scale=True, unit='B')
        self.s3.upload_file(
            pkg_path, lambda_bucket, self.get_s3_name(), Callback=progress.update)

        Oprint.info('Finished uploading ' + self.get_pkg_name() +
                    ' to S3 bucket ' + self.config_loader.get_value('LambdaBucketName'), 's3')
    ...
    def destroy(self):
        ...
        try:
            Oprint.info('Start deleting ' + self.get_pkg_name() + ' from S3 bucket ' +
                        self.config_loader.get_value('LambdaBucketName'), 's3')

            self.s3.delete_object(Bucket=self.config_loader.get_value(
                'LambdaBucketName'), Key=self.get_s3_name())

            Oprint.info(self.get_pkg_name() + ' has been deleted from S3 bucket ' +
                        self.config_loader.get_value('LambdaBucketName'), 's3')
